...
export function FoldedMiniPreview({
  entry,
  miniSize,
  resolveEntry,
  standBufferMm,
  ...
  const previewKey = JSON.stringify({ entry, miniSize, standBufferMm });
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [entry, forcePlaceholder, miniSize, previewKey, resolveEntry, standBufferMm]);
